...
export default function Home() {
    ...
    return (
        <>
            <button style={{marginTop: '20px'}} onClick={() => route.push('/')}>Go Back</button>
            {InfoLoad ? (
            ...
            <ul className='tab'>
                <li><button className={(tab) ? "on" : ""} onClick={(el)=>handleTab(el)}>{"Chart"}</button></li>
                <li><button className={(!tab) ? "on" : ""} onClick={(el)=>handleTab(el)}>{"Price"}</button></li>
            </ul>
            <a/>
